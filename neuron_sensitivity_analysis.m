...
date = 28;
date = num2str(date);
cortical_areas = {'M1U'; 'M1F'; 'S1U'; 'S1F'};
...
% %     title(sprintf('Region of Peak Geometric Index for %s Neurons', cortical_areas{area}));
...
% % % END FOR LOOP
% % 
% % % GRAPH FR BEFORE/AFTER AND DURING - SCATTER
% % neuron = 1;
% % for trial=1:70
% %     hold on
% %     scatter(fr_before_after{neuron}{trial},fr_during{neuron}{trial})
% % end
% % ylabel('FR During Contact Event, Hz')
% % xlabel('FR Before and After Contact Event, Hz')
% % ylim([0,max(max(fr_before_after{neuron}{trial},fr_during{neuron}{trial}))+2])
% % xlim([0,max(max(fr_before_after{neuron}{trial},fr_during{neuron}{trial}))+2])
% % hold on 
% % x = linspace(0,300);
% % y = linspace(0,300);
% % plot(x,y)
...
%% STACKED BAR GRAPHS
load('table.mat')
H = bar(table,'stacked','FaceColor','flat');
title('Palatal Region of Contact Associated with the Highest Change in Firing Rate by Population')
legend('Ant. R','Ant. L','Med. R','Med. L','Pos. R','Pos. L')
set(gcf,'color','w');
ylabel('Percentage of Population')
xticks([1.5,4.5,7.5,10.5])
xticklabels({'Rostral M1o','Caudal M1o','Areas 1/2','Areas 3a/3b'})
set(H(1),'FaceColor','g')
%H(1,1).FaceColor='flat';
H(1).CData = [0.3 0.3 0.7];
H(2).CData = [1 1 1]*0.8;
% H(2).FaceColor=;
% H(3).FaceColor='c';
% H(4).FaceColor='c';
